...
try:
    ...
    href_line_new = []
    # 定義要爬取的網址
    url = ["https://www.hp.com/us-en/shop/sitesearch?keyword=Desktops","https://www.hp.com/us-en/shop/sitesearch?dte_facet_category=Laptops&keyword=workstation"]
    url_number = 0
    for url_number in range(len(url)):
        my_header = {'user-agent':UserAgent().random}
        #開啟搜尋頁面
        HP_DT = webdriver.Chrome()
        HP_DT.get(url[url_number])
    ...
    for i in range(len(href_line)):
        ...
        sleep(2)
        while "無法連上這個網站" in HP_DT_data.page_source:
            HP_DT_data.quit()
            sleep(2)
            new_ip = random.choice(proxy_data)[0]
            random_proxy = new_ip
