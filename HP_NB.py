...
try:
    url = ["https://www.hp.com/us-en/shop/sitesearch?keyword=Laptops","https://www.hp.com/us-en/shop/sitesearch?dte_facet_category=Desktops&keyword=workstation"]
    ID_link = []
    Money_link = []
    href_line = []
    url_number = 0
    for url_number in range(len(url)):
        ...
        HP_NB = webdriver.Chrome(options=option)
        HP_NB.get(url[url_number])
    ...
    for i in range(len(href_line)):
        ...
        sleep(2)
        while "無法連上這個網站" in HP_NB_data.page_source: 
            HP_NB_data.quit()
            sleep(2)
            new_ip = random.choice(proxy_data)[0]
            random_proxy = new_ip
        ...
        retext=0
        #針對不同網頁寫法抓取
        while (len(L_NB_data_n) < 1 and retext == 6):
        ...
        for ack in L_NB_data_d_ack:
            ...
            if L_NB_data_n[j].text == "Display" or L_NB_data_n[j].text == "Memory" or L_NB_data_n[j].text == "Processor":
                ...
                B.append(N_D) 
            elif L_NB_data_n[j].text == "Internal storage" and "Hard Drive" not in A:
                A.append("Hard Drive")
                B.append(N_D)
                ...
                if len(N_D.split("+"))>3 and "Hard Drive" not in A:
                    A.append("Hard Drive")
                ...
                B.append(N_D)
            elif (L_NB_data_n[j].text == "Storage" or L_NB_data_n[j].text == "Hard drive" or  L_NB_data_n[j].text == "Internal drive") and "Hard Drive" not in A:
                A.append("Hard Drive")
                B.append(N_D)
