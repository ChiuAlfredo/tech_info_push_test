...
def make_request_with_proxy(url, proxy):
    ...
    try:
        response = requests.get(url, headers=my_header, proxies=proxies, timeout=5)
    ...
    for i in range(len(href_line)):
        ...
        while True:
                ...
                if html_content is not None:
                    L_DT_soup = BeautifulSoup(html_content, 'html.parser')
                    sleep(4)    
                    #抓取特徵名稱
        ...
        if len(DT_deta_url) > 0:
            ...
            Lenovo_DT_data_deta = webdriver.Chrome(options = option)            
            Lenovo_DT_data_deta.get("https://www.lenovo.com" + DT_deta_url[0]['href'] + "#features")
            
            while "Access Denied" in Lenovo_DT_data_deta.page_source:
